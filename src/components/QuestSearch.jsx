...
export default function QuestSearch({ tasks, mine, onAdd, onRemove, loading }) {
  ...
  const hits = q.length >= 1
    ? tasks
        .filter(t => t.name.toLowerCase().includes(q.toLowerCase()) && !mine.find(x => x.id === t.id))
        .slice(0, 14)
    : []
    ...
    <div style={{ position: 'relative' }}>
        ...
        <div style={{
          ...
          {hits.map(t => (
            <div
              ...
              style={{
                padding: '9px 12px', cursor: 'pointer', borderBottom: '1px solid var(--brd)',
            ...
            >
              <div>
                ...
                <div className="mono" style={{ fontSize: 11, color: 'var(--txm)' }}>
                  ...
                  {!t.map && <span style={{ marginLeft: 6, color: 'var(--txd)' }}>ANY MAP</span>}
                </div>
              </div>
      ...
      {open && q.length >= 1 && hits.length === 0 && !loading && (
        <div style={{
          ...
          NO QUESTS FOUND FOR "{q.toUpperCase()}"
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
        {mine.map(item => (
          <div key={item.id} style={{
            ...
            background: 'var(--sur2)', border: '1px solid var(--golddim)', borderRadius: 4, fontSize: 12,
          }}>
            <span style={{ color: 'var(--goldtx)' }}>{item.name}</span>
            <button
              onMouseDown={() => onRemove(item.id)}
              style={{ background: 'none', border: 'none', color: 'var(--txm)', padding: 0, fontSize: 16, lineHeight: 1, cursor: 'pointer' }}
            >×</button>
          </div>
        ))}
        {mine.length === 0 && !loading && (
          <p className="mono" style={{ fontSize: 11, color: 'var(--txd)' }}>
            TYPE TO SEARCH AND ADD YOUR ACTIVE QUESTS
          </p>
        )}
      </div>
    </div>
