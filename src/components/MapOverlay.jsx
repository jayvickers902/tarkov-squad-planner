...
import { SPAWNS, TERRAIN, TERRAIN_LABELS } from '../constants'
...
export default function MapOverlay({ mapNorm, mapName, tasks, memberQuests, spawn, onSetSpawn, isLeader }) {
  ...
  // tarkov.dev serves map images — works fine outside the Claude sandbox
  const imgSrc = `https://assets.tarkov.dev/maps/${mapNorm}-2d.jpg`
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        background: '#08100e', border: '1px solid var(--brd)', borderRadius: 6, overflow: 'hidden',
      }}>
        {/* Real map image from tarkov.dev */}
        {imgOk && (
          <img
            src={imgSrc}
            alt={mapName}
        ...
        {/* SVG fallback terrain + overlay (always rendered, image sits underneath) */}
        <svg
          ref={svgRef}
        ...
        >
          {/* Only show drawn terrain if image failed to load */}
          {!imgOk && (
            <>
              ...
              {(TERRAIN[mapNorm] || []).map((z, i) => (
                <rect key={i} x={z.x} y={z.y} width={z.w} height={z.h} rx="1" fill={z.fill || '#1a2e22'} opacity={z.op || 0.6} />
              ))}
              {(TERRAIN_LABELS[mapNorm] || []).map((l, i) => (
                <text key={i} x={l.x} y={l.y} textAnchor="middle" fill="#2a4a35" fontSize="3" fontFamily="Share Tech Mono" opacity="0.7">
                  {l.label}
                </text>
              ))}
            </>
          ...
          {route.length > 1 && route.map((p, i) => i > 0 ? (
            <line key={i}
              ...
              x2={p.px} y2={p.py}
              stroke="#c9a84c" strokeWidth="0.5" strokeDasharray="1.5 0.8" opacity="0.9"
            />
          ) : null)}
          ...
          {objs.map(({ obj, task, px, py }, i) => {
            ...
            return (
              <g key={obj.id} style={{ cursor: 'pointer' }}
                ...
                <circle cx={px} cy={py} r={isHov ? 1.8 : 1.3} fill={c} opacity={isHov ? 1 : .9} />
                <text x={px} y={py + 3.2} textAnchor="middle" fill={c} fontSize="2" fontFamily="Share Tech Mono">{i + 1}</text>
              </g>
            )
          })}
          ...
          {/* Spawn markers */}
          {spawns.map(s => {
            const sx = s.x * 100, sy = s.y * 100
            const sel = spawn === s.id
            return (
              <g key={s.id}
                style={{ cursor: isLeader ? 'pointer' : 'default' }}
                onClick={() => { if (isLeader) onSetSpawn(sel ? null : s.id) }}>
                <circle cx={sx} cy={sy} r={sel ? 2.2 : 1.5}
                  fill={sel ? '#c9a84c' : 'rgba(201,168,76,0.15)'}
                  stroke="#c9a84c" strokeWidth={sel ? .6 : .4} />
                {sel && <circle cx={sx} cy={sy} r=".8" fill="var(--bg)" />}
                <text x={sx} y={sy - (sel ? 3.2 : 2.6)} textAnchor="middle"
                  fill={sel ? '#e8c96a' : '#c9a84c'}
                  fontSize={sel ? 1.8 : 1.5}
        ...
        {hov && (
          <div style={{
            ...
            background: 'rgba(8,16,14,0.95)', border: '1px solid var(--brd2)',
            borderRadius: 4, padding: '7px 10px', fontSize: 12, pointerEvents: 'none',
          }}>
            <div className="mono" style={{ color: 'var(--gold)', marginBottom: 2, fontSize: 10 }}>{hov.task.name}</div>
      ...
      <div style={{ marginTop: 10 }}>
        <div className="lbl">
          {isLeader ? 'CLICK MAP OR BUTTON TO SET SPAWN' : 'SPAWN LOCATION (SET BY LEADER)'}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {spawns.map(s => (
            <button
              ...
              style={{ padding: '4px 9px', fontSize: 11, opacity: isLeader ? 1 : .6, cursor: isLeader ? 'pointer' : 'default' }}
            >
              {s.label}
            </button>
          ))}
        ...
        {spawn && objs.length > 0 && (() => {
          ...
          return (
            <div className="mono" style={{
              ...
              ◈ ROUTE OPTIMIZED // {objs.length} OBJ // NEAREST-FIRST FROM {(sp?.label || spawn).toUpperCase()}
            </div>
          )
        })()}
        ...
        {!objs.length && (
          <p className="mono" style={{ marginTop: 8, fontSize: 11, color: 'var(--txd)' }}>
            ADD QUESTS TO SEE OBJECTIVE MARKERS
          </p>
        )}
      </div>
    </div>
