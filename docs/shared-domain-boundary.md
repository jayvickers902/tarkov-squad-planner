...
`shared/companionSyncEngine.js`, `shared/pingCadence.js` and `shared/taskCatalog.js` are the three
facades at the `shared/` root — the paths both companion and web code import. The companion sync
engine's implementation and its pure parser dependency graph live under `shared/domain/`, alongside
twelve more framework-free modules (`constants.js`, `partyMembers.js`, `settings.js`, `tarkovPings.js`,
`tarkovObjectives.js`, `questWipe.js`, `questLogState.js`, `eftLogDirectory.js`, `eftScreenshots.js`,
`eftLogs.js`, plus map data in `mapFloors.js` / `tarkovMapConfigs.js`) that back 2-line re-export
